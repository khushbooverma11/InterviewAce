...
// Port used exclusively for the Metro bundler during production builds.
// 8080 = API server, 8081 = mockup-sandbox dev server, so use 8083.
...
async function buildWeb(domain, expoPublicReplId) {
  console.log('\n--- Building web export (expo export --platform web) ---');
  ...
  await new Promise((resolve, reject) => {
    const proc = spawn(
      'pnpm',
      ['exec', 'expo', 'export', '--platform', 'web', '--output-dir', webOutputDir, '--clear'],
      {
        stdio: 'inherit',
        cwd: projectRoot,
        env,
...
async function main() {
  console.log('Building static Expo deployment (web + Expo Go)...');
  ...
  // ── 1. Web export (served to browsers) ──────────────────────────────────
  await buildWeb(domain, expoPublicReplId);

  // ── 2. Expo Go mobile bundles (served to Expo Go via QR code) ───────────
  ...
  console.log('\nBuild complete! Deploy to:', baseUrl);
